...
void ques_finished (qk_tap_dance_state_t *state, void *user_data) {
  xtap_state.state = cur_dance(state);
  switch (xtap_state.state) {
    case SINGLE_TAP: register_code(KC_SLASH); break;
    case DOUBLE_TAP: register_code(KC_LSHIFT); register_code(KC_SLASH); unregister_code(KC_SLASH); break;
    case DOUBLE_SINGLE_TAP : register_code(KC_LSHIFT); register_code(KC_SLASH); unregister_code(KC_SLASH); break;
    //Last case is for fast typing. Assuming your key is `f`:
    //For example, when typing the word `buffer`, and you want to make sure that you send `ff` and not `Esc`.
    //In order to type `ff` when typing fast, the next character will have to be hit within the `TAPPING_TERM`, which by default is 200ms.
  }
...
void ques_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (xtap_state.state) {
    case SINGLE_TAP: unregister_code(KC_SLASH); break;
    case DOUBLE_TAP: unregister_code(KC_LSHIFT); break;
    case DOUBLE_SINGLE_TAP : unregister_code(KC_LSHIFT); break  ;
  }
  xtap_state.state = 0;
}

qk_tap_dance_action_t tap_dance_actions[] = {
  [TAB_ESC] = ACTION_TAP_DANCE_FN_ADVANCED(NULL,esc_tab_finished, esc_tab_reset),
  [QUES_SLASH] = ACTION_TAP_DANCE_FN_ADVANCED(NULL,ques_finished, ques_reset),
};